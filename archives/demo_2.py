...
def draw_3d_axes(ax, color='gray'):
    ax_lim = 1.2 * np.max(np.abs([*ax.get_xlim(), *ax.get_ylim(), *ax.get_zlim()]))
    ax.plot([-ax_lim, ax_lim], [0, 0], [0, 0], color=color)
    ax.plot([0, 0], [-ax_lim, ax_lim], [0, 0], color=color)
    ax.plot([0, 0], [0, 0], [-ax_lim, ax_lim], color=color)
